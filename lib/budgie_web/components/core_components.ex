defmodule BudgieWeb.CoreComponents do
  @moduledoc """
  ...
  def dropdown_menu(assigns) do
    ...
    >
      ...
      <div
        id={"dropdown-#{@id}-body"}
        class="absolute left-0 z-10 hidden w-48 bg-white border border-gray-200 rounded-lg shadow-lg top-12"
      >
        <div class="py-1">
          {render_slot(@inner_block)}
        </div>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BudgieWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BudgieWeb.Gettext, "errors", msg, opts)
    end
  end
